...
function upgradeSword(attribute) {
  ...
  if (attribute === 'rarity') {
    ...
    if (currentIndex < rarities.length - 1) {
      ...
      const upgradeChance = nextRarity.baseChance / luckValue;
    ...
    if (currentIndex < molds.length - 1) {
      ...
      const upgradeChance = nextMold.baseChance / luckValue;
      if (Math.random() < upgradeChance) {
    ...
    if (currentIndex < qualities.length - 1) {
      ...
      const upgradeChance = nextQuality.baseChance / luckValue;
      if (Math.random() < upgradeChance) {
